...
module dla_hld_ram_lower #(
    ...
    // If this is a TOP/MID depth stitched RAM the address can sometimes, legally, be out of range
    // when the other depth section (MID/BOT) are being used.
    // This bit can be set for simulation to filter out altera_syncram warnings
    parameter  bit PREVENT_SYNCRAM_ADDR_WARNINGS = 1'b0,
    ...
    `DLA_ACL_PARAMETER_ASSERT_MESSAGE(RAM_BLOCK_TYPE_IS_M20K || RAM_BLOCK_TYPE_IS_MLAB, $sformatf("Illegal RAM_BLOCK_TYPE: %s.", RAM_BLOCK_TYPE))
    ...
    `DLA_ACL_PARAMETER_ASSERT_MESSAGE(RAM_OPERATION_MODE_IS_SIMPLE_DUAL_PORT || RAM_OPERATION_MODE_IS_TRUE_DUAL_PORT, $sformatf("Illegal RAM_OPERATION_MODE: %s.", RAM_OPERATION_MODE))
    ...
    // If this is a TOP/MID depth stitched RAM the address can sometimes, legally, be out of range
    // when the other depth section (MID/BOT) are being used.
    logic [ADDR-1:0] w_a_address;
    logic [ADDR-1:0] w_b_address;
    if (PREVENT_SYNCRAM_ADDR_WARNINGS) begin : GEN_PREVENT_SYNCRAM_ADDR_WARNINGS
      assign w_a_address = (a_read_enable | a_write | (a_address < DEPTH)) ? a_address : DEPTH-1;
      assign w_b_address = (b_read_enable | b_write | (a_address < DEPTH)) ? b_address : DEPTH-1;
    end else begin : GEN_ADDRESS_WIRE
      assign w_a_address = a_address;
      assign w_b_address = b_address;
    end
