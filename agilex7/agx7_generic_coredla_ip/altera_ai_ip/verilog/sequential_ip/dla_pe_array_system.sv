...
module dla_pe_array_system import dla_common_pkg::*; #(
  ...
  for (genvar pe_port = 0; pe_port < SCRATCHPAD_ARCH.NUM_PE_PORTS; pe_port++) begin
    ...
    assign pe_array_scale.data[pe_idx].scale[filter_idx] = scratchpad_read_data.data.ports[pe_port].scale;
    assign pe_array_bias.data[pe_idx].bias[filter_idx]  = scratchpad_read_data.data.ports[pe_port].bias;
    if (filter_idx == 0) begin
      assign pe_array_scale.data[pe_idx].valid = scratchpad_read_data.data.ports[pe_port].valid;
      assign pe_array_bias.data[pe_idx].valid  = scratchpad_read_data.data.ports[pe_port].valid;
    end
  end
  ...
  if (SCRATCHPAD_ARCH.ENABLE_DDRFREE_FBS) begin: gen_ddr_free_fbs_scratchpad
    dla_filter_bias_scale_scratchpad #(SCRATCHPAD_ARCH) scratchpad (
        ...
        .i_read        ((PE_ARRAY_ARCH.DOT_MODE == dla_pe_array_pkg::DOT_MODE_TENSOR1X2_MULT8) ? scratchpad_read : 1'b1),
  ...
  for (genvar lane_idx = 0; lane_idx < PE_ARRAY_ARCH.NUM_LANES; lane_idx++) begin : GEN_LANE_ASSIGN
    ...
    dla_delay #(
      ...
      .DELAY_PRIMITIVE("LOGIC"),
      .DEVICE(PE_ARRAY_ARCH.DEVICE_FAMILY)
    ) feature_data_reg (
      .clk(clk),
