...
#(
  ...
  int DDR_WRITE_ID_WIDTH,           // width of the AXI ID signal for DDR writes, must be between 1 and 8
  ...
  int LAYOUT_TRANSFORM_NUM_CHANNELS,
  ...
  int LAYOUT_TRANSFORM_WRITEBACK_MODE=0
) (
  ...
  // Config intercept signals for on-chip parameter mode (runtime address offsets from CSR to config network)
  logic                                              on_chip_param_intercept_config_valid;
  logic               [8*CONFIG_DATA_BYTES-1:0]      on_chip_param_intercept_config_data;
  logic                                              on_chip_param_intercept_config_ready;
  ...
  // Output streamer license flags and error signals
  logic                                              ostream_license_flag;
  logic                                              ostream_stream_error_axi_clk, ostream_stream_error_ddr_clk;

  logic                                    lt_wb_awvalid;
  logic               [DDR_ADDR_WIDTH-1:0] lt_wb_awaddr;
  logic              [DDR_BURST_WIDTH-1:0] lt_wb_awlen;
  logic         [AXI_BURST_SIZE_WIDTH-1:0] lt_wb_awsize;
  logic         [AXI_BURST_TYPE_WIDTH-1:0] lt_wb_awburst;
  logic                                    lt_wb_awready;
  logic                                    lt_wb_wvalid;
  logic             [8*DDR_DATA_BYTES-1:0] lt_wb_wdata;
  logic               [DDR_DATA_BYTES-1:0] lt_wb_wstrb;
  logic                                    lt_wb_wlast;
  logic                                    lt_wb_wready;
  logic                                    lt_wb_bvalid;
  logic                                    lt_wb_bready;
  logic                                    dma_lt_wb_read_done;


  logic [DDR_ADDR_WIDTH-1:0] wb_read_addr;
  logic wb_read_addr_valid, wb_read_addr_ready;
  ...
  logic [INPUT_FEEDER_LANE_DATA_WIDTH-1:0] istream_data; // Gets assigned to one of the two below
  logic [INPUT_FEEDER_LANE_DATA_WIDTH-1:0] wb_istream_data;
  logic [INPUT_FEEDER_LANE_DATA_WIDTH-1:0] if_istream_data;
  logic istream_ready, pe_stream_ready, wb_stream_ready;
  logic istream_valid;
  logic done_write_ddr, done_write_dla;

  logic [DDR_ADDR_WIDTH-1:0] wb_base_addr;
  logic [DDR_ADDR_WIDTH-1:0] wb_frame_size;
  logic [DDR_ADDR_WIDTH-1:0] wb_addr_range;
  logic wb_start;
  ...
  if (ENABLE_INPUT_STREAMING) begin
    ...
    logic stream_output_clk;
    assign stream_output_clk = LAYOUT_TRANSFORM_WRITEBACK_MODE ? clk_ddr : clk_dla;
    ...
    if (LAYOUT_TRANSFORM_WRITEBACK_MODE) begin
      assign wb_istream_data = istream_data;
      assign if_istream_data = '0;
      dla_external_buffer_manager #(
        ...
        // .clk_axi(clk_axi),
        .i_resetn_async(w_resetn_async),
        .i_base_address(wb_base_addr), // Needs to come from CSR
        .i_frame_size(wb_frame_size), // CSR
        .i_address_range(wb_addr_range), // CSR
        .i_ready_to_start(wb_start), // CSR

        // Streaming data incoming from the input streamer (post-LT) - needs to be arbitrated
        // with the PE array, which consumes this data in other modes.
        .i_wb_data(wb_istream_data),
        .i_wb_valid(istream_valid),
        .o_wb_ready(wb_stream_ready),

        .o_ddr_awvalid(lt_wb_awvalid),
        .o_ddr_awaddr(lt_wb_awaddr),
        .o_ddr_awlen(lt_wb_awlen),
        .i_ddr_awready(lt_wb_awready),
        .o_ddr_wvalid(lt_wb_wvalid),
        .o_ddr_wdata(lt_wb_wdata),
        .o_ddr_wstrb(lt_wb_wstrb),
        .o_ddr_wlast(lt_wb_wlast),
        .i_ddr_wready(lt_wb_wready),
        .i_ddr_bvalid(lt_wb_bvalid),
        .o_ddr_bready(lt_wb_bready),

        // Read address signals, sent to the DMA CSR to read the feature data
        .o_read_addr(wb_read_addr),
        .o_read_addr_valid(wb_read_addr_valid),
        .i_read_addr_ready(wb_read_addr_ready),
        .i_read_finished(dma_lt_wb_read_done)
      );
      assign istream_ready = wb_stream_ready;
    end else begin
      assign if_istream_data = istream_data;
      assign wb_istream_data = '0;
      assign istream_ready = pe_stream_ready & dla_core_streaming_active;
    end
  ...
  */
  ...
  assign csr_scratchpad_update_addr_if.data.mem_id    = '0;
  ...
  dla_dma #(
    ...
    .DDR_WRITE_ID_WIDTH             (DDR_WRITE_ID_WIDTH),
    .DEVICE                         (DEVICE_ENUM),
    .LT_ARCH                        (LT_ARCH),
    .LW_LT_ARCH                     (LW_LT_ARCH),
  ...
  (
    ...
    .i_ostream_is_licensed          (ostream_license_flag), // indicates whether the output stream is licensed
    .i_ostream_stream_error         (ostream_stream_error_ddr_clk), // indicates whether there is an error in the output stream
    ...
    .o_on_chip_param_intercept_config_valid (on_chip_param_intercept_config_valid),
    .o_on_chip_param_intercept_config_data  (on_chip_param_intercept_config_data),
    .i_on_chip_param_intercept_config_ready (on_chip_param_intercept_config_ready),
    ...
    //Layout transformed data to be written back to DDR
    .i_lt_wb_awvalid                (lt_wb_awvalid),
    .i_lt_wb_awaddr                 (lt_wb_awaddr),
    .i_lt_wb_awlen                  (lt_wb_awlen),
    .o_lt_wb_awready                (lt_wb_awready),
    .i_lt_wb_wvalid                 (lt_wb_wvalid),
    .i_lt_wb_wdata                  (lt_wb_wdata),
    .i_lt_wb_wstrb                  (lt_wb_wstrb),
    .i_lt_wb_wlast                  (lt_wb_wlast),
    .o_lt_wb_wready                 (lt_wb_wready),
    .o_lt_wb_bvalid                 (lt_wb_bvalid),
    .i_lt_wb_bready                 (lt_wb_bready),

    .o_lw_tb_read_last              (dma_lt_wb_read_done),

    .o_wb_base_addr                 (wb_base_addr),
    .o_wb_frame_size                (wb_frame_size),
    .o_wb_addr_range                (wb_addr_range),
    .o_wb_start                     (wb_start),

    .i_read_addr(wb_read_addr),
    .i_read_addr_valid(wb_read_addr_valid),
    .o_read_addr_ready(wb_read_addr_ready),
    ...
    .o_done_write                   (done_write_ddr)
  );

  dla_clock_cross_full_sync cc_core_start_streaming (
    .clk_src(clk_ddr),
    .i_src_data(done_write_ddr),
    .o_src_data(),

    .clk_dst(clk_dla),
    .o_dst_data(done_write_dla)
  );
  ...
  dla_pe_array_system #(
    ...
    // If writeback is enabled, these are connected to DDR via the external buffer manager.
    .i_istream_data              (if_istream_data),
    .o_istream_ready             (pe_stream_ready),
    .i_istream_valid             (istream_valid & dla_core_streaming_active & !LAYOUT_TRANSFORM_WRITEBACK_MODE),
  ...
  if (ENABLE_OUTPUT_STREAMER == 1) begin
    ...
    dla_output_streamer # (
      .MAX_TRANSACTIONS     ( (STREAM_BUFFER_DEPTH * PE_ARRAY_ARCH.NUM_LANES * PE_ARRAY_ARCH.DOT_SIZE * 16) / AXI_OSTREAM_DATA_WIDTH ),
      .CONFIG_WIDTH         ( CONFIG_WIDTH              ),
      ...
      .o_axi_t_user         (                                                              ),
      .o_axi_t_wakeup       (                                                              ),
      .o_license_flag       ( ostream_license_flag                                         ),
      .o_streamer_error     ( ostream_stream_error_axi_clk                                 )
    );
    ...
    dla_demux # (
      ...
      .i_transmitter_done(xbar_input_done | done_write_dla),
    ...
    dla_clock_cross_full_sync #(
      // Rely on default parametrization
    ) cc_ostream_error (
      .clk_src            (clk_axi),
      .clk_dst            (clk_ddr),
      .i_src_data         (ostream_stream_error_axi_clk),
      .o_dst_data         (ostream_stream_error_ddr_clk),
      .o_src_data         ()
    );

    // No need to synchronize the output streamer's license flag since it is tied to a constant value.
    ...
    assign ostream_license_flag = 1'b1;
    assign ostream_stream_error_axi_clk = 1'b0;
    assign ostream_stream_error_ddr_clk = 1'b0;
  ...
  dla_config_network #(
    ...
    .ENABLE_ON_CHIP_CONFIG          (ENABLE_ON_CHIP_PARAMETERS),
    .ENABLE_INPUT_STREAMING         (ENABLE_INPUT_STREAMING),
    .ENABLE_OUTPUT_STREAMING        (ENABLE_OUTPUT_STREAMER),
    .DEVICE                         (DEVICE_ENUM),
    .CONFIG_DATA_BYTES              (CONFIG_DATA_BYTES),
    ...
    // Config intercept interface for on-chip parameter mode
    .i_intercept_config_valid       (on_chip_param_intercept_config_valid),
    .i_intercept_config_data        (on_chip_param_intercept_config_data),
    .o_intercept_config_ready       (on_chip_param_intercept_config_ready),
