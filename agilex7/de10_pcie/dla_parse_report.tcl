...
namespace eval DLA {
    ...
    proc get_clock_info { clocks_arr } {
    #
    ...
    # --------------------------------------------------------------------------
        ...
        set ip_clock        [find_row_and_get_value $fmax_summary_id "Clock Name" "Fmax" $clocks(ip)]

        # Package for output
        set frequencies(platform) [parse_report_value "string" $platform_clock]
        set frequencies(ip_fmax)  [parse_report_value "string" $ip_clock]
    ...
    proc write_json_report { project_name clocks_arr output_file } {
    #
    # Description: Write the JSON resource usage report for the given project.
    #
    # Parameters:
    #    project_name - project/revision name (usually, but not always, 'top')
    ...
    # --------------------------------------------------------------------------
        ...
        array set clock_info [get_clock_info clocks]
...
# ------------------------------------------------------------------------------
    ...
    set options {
        {"project.arg" "top" "Project revision name"}
    ...
    ::DLA::write_json_report $project_name clocks $::DLA::COMPILE_REPORT_JSON
}
